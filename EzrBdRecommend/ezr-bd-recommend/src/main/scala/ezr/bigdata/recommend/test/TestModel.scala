...
object TestModel {
  def main(args: Array[String]):Unit= {
    ...
    val spark = SparkSession
      ...
      .appName("RankItem pro"+shardingGrpId)
      .master("local[*]")
      .enableHiveSupport()
      .getOrCreate()
    import spark.implicits._
    //  获取用户行为数据  商品详情数据  热门商品
    val sql_u = "select vipid as user_id,itemno as item_idI,activetype as behavior,daystime from pro"+shardingGrpId+".user_item_behavior"
    val sql_p = "select itemno as item_id,name as item_name,saleprice from pro"+shardingGrpId+".mall_prd_prod"
    val sql_h = "select item_id,item_hot from pro"+shardingGrpId+".recall_hot_result order by item_hot desc limit 10"
    //转换获得的时间类型数据
    val behavior = spark.sql(sql_u).withColumn("daystime",col("daystime").cast(DateType))
    ...
    train_set = train_set.join(item_id_feture,label("item_id") === item_id_feture("item_id_behavior"),"left").drop(item_id_feture("item_id_behavior"))
    //特征列名重复的重命名  并把null替换为0
    train_set = GetFeture.RenameDataFrame(train_set,spark).na.fill(0)

   //2 训练样本准备
   //    val data = spark.read.format("libsvm").load("/data/sample_libsvm_data.txt")
//    val data = MLUtils.loadLibSVMFile(spark.sparkContext,"/data/sample_libsvm_data.txt")
//    val splits = data.randomSplit(Array(0.8, 0.2))
//    val (train, test) = (splits(0), splits(1))
//    val train_set = data.toDF().show()
    //转换特征数据为Double型
